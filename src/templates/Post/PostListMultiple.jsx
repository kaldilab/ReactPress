...
import * as Navs from 'components/Navs';
...
export default function PostListMultiple(props) {
  ...
  const searchQueryCategory = searchQuery.category ? searchQuery.category : '';
  ...
  const [postsNavs, setPostsNavs] = useState([]);
  ...
  const [postsCategory, setPostsCategory] = useState(searchQueryCategory);
  ...
  useEffect(
    () => {
      ...
      const storeKey = 'Content' + routeUrlReplace + '_' + postsCategory + '_PostsNavs';
      const setResponse = (data) => {
        if (isMounted.current) {
          setPostsNavs(data);
        }
      }
      const getPostsNavs = async () => {
      ...
      GetStorage.CallSessionStore(storeKey, setResponse, getPostsNavs);
      setPostsCategory(searchQueryCategory);
      return () => {
        isMounted.current = false;
      }
    },
    [routeUrlReplace, postsCategory, searchQueryCategory]
  );
  ...
  useEffect(
    () => {
      isMounted.current = true;
      const storeKey = 'Content' + routeUrlReplace + '_' + postsCategory + '_PostsSticky';
      const setResponse = (data) => {
        if (isMounted.current) {
          setPostsSticky(data);
        }
  ...
  useEffect(
    () => {
      isMounted.current = true;
      const storeKeyPrefix = 'Content' + routeUrlReplace + '_' + postsCategory + '_' + newLookupValue;
    ...
    [routeUrl, routeUrlReplace, postsCurrentPage, newLookupValue, postsPerPage, postsForcePage, postsCategory]
  );

  return (
    <React.Fragment>
      <h3 className="mb-5">{GetMenu.MenuLabel(routeSlug)}</h3>
      <p className="mb-5 font-italic text-right text-warning">워드프레스 전체 포스트를 카테고리별로 출력하는 페이지입니다.</p>
      <Section loading={isLoadingSection} sectionClass={'section'} srTitle={GetMenu.MenuLabel(routeSlug)}>
        <Navs.CTNavs navClass={'mb-5'}>
          <Navs.CTNavsItem
            active={postsCategory ? '' : 'active'}
            link={routeUrl}
            title={'전체'}
          />
          {
            ...
            postsNavs
              .map((item, index) => {
                return (
                  <Navs.CTNavsItem
                    key={index}
                    active={(postsCategory === item.slug) ? 'active' : ''}
                    link={routeUrl + '?category=' + item.slug}
                    title={item.name}
                  />
                )
              })
                    ...
                    .map((item, index) => {
                      ...
                      return (
                        <BoardCard.CTBoardCardItem
                          ...
                          excerpt={GetUtils.GetEllipsis(GetUtils.GetMarkupRemove(item.excerpt.rendered), 100, '')}
                          date={GetDate.FormatDate(item.date)}
                        />
                      )
                    })
